...
namespace ModularAudience.Forms.Modules.Dialogs
{
    ...
    {
        private readonly MidiFileData midiFile;
        private readonly List<MidiTrackData> tracks;
        private AudioObj? customSample;
        private AudioObj? previewAudio;
        private CancellationTokenSource? previewCts;

        public MidiWindow(string filePath)
        {
            this.InitializeComponent();
            this.midiFile = MidiFileData.Load(filePath);
            this.tracks = this.midiFile.Tracks.ToList();
        this.Text = $"MIDI Renderer - {Path.GetFileName(filePath)}";
        ...
        private MidiInstrument SelectedInstrument => (MidiInstrument)Math.Clamp(this.comboBox_instrument.SelectedIndex, 0, 6);
        ...
        private async void button_save_Click(object? sender, EventArgs e)
        {
            ...
            this.label_status.Text = "Rendering MIDI...";
                AudioObj audio = await Task.Run(() => MidiAudioRenderer.Render(this.midiFile, this.SelectedTrack.Index, this.SelectedInstrument, (double)this.numericUpDown_bpm.Value, this.customSample));
