...
namespace ModularAudience.Forms
{
    public partial class AudioCollectionView : Form
    {
        ...
        private List<AudioObj> _stableSelection = [];
        private List<AudioObj>? _dragSelection;
        private AudioObj? _clickedAudio;
        private bool _dragStarted;
        private bool _collapseSelectionOnClick;
        ...
        public AudioCollectionView(IEnumerable<AudioObj> audios, string? title = null)
        {
            ...
            this.listBox_audios.KeyDown += this.listBox_audios_KeyDown;
        ...
        private void listBox_audios_MouseDown(object? sender, MouseEventArgs e)
        {
            ...
            this._collapseSelectionOnClick = false;
            ...
            else if (e.Button == MouseButtons.Left)
            {
                ...
                if (index != ListBox.NoMatches)
                {
                    this._clickedAudio = (AudioObj)this.listBox_audios.Items[index];
                    this._collapseSelectionOnClick = (ModifierKeys & (Keys.Control | Keys.Shift)) == Keys.None;
                    if (this._collapseSelectionOnClick && this._stableSelection.Contains(this._clickedAudio))
                    {
                        this._dragSelection = [.. this._stableSelection];
                        if (this._dragSelection.Count > 1)
                        {
                            this.RestoreListBoxSelection(this._dragSelection);
                        }
                    }
        ...
        private void listBox_audios_MouseUp(object? sender, MouseEventArgs e)
        {
            if (e.Button == MouseButtons.Left
                && this._collapseSelectionOnClick
                && !this._dragStarted
                && this._clickedAudio != null)
            {
        ...
        private void RestoreListBoxSelection(IEnumerable<AudioObj> audios)
        {
            for (int index = this.listBox_audios.SelectedIndices.Count - 1; index >= 0; index--)
            {
                this.listBox_audios.SetSelected(this.listBox_audios.SelectedIndices[index], false);
            }

            foreach (AudioObj audio in audios)
            {
                int index = this.listBox_audios.Items.IndexOf(audio);
                if (index >= 0)
                {
        ...
        private void listBox_audios_MouseMove_DragStart(object? sender, MouseEventArgs e)
        {
            ...
            if (this._dragSelection is { Count: > 1 } dragSelection)
            {
                this.RestoreListBoxSelection(dragSelection);
            }
        ...
        private async void listBox_audios_MouseClick(object? sender, MouseEventArgs e)
        {
            ...
            if (this.listBox_audios.Items[index] is AudioObj audio)
            {
                ...
                {
                    await this.TriggerAutoPlayAsync(audio, toggleSameTrack: true).ConfigureAwait(false);
                }
            }
        ...
        {
            if (this.listBox_audios.IsHandleCreated && !this.listBox_audios.IsDisposed)
            {
                try
                {
                    this.listBox_audios.BeginInvoke((Action)(() =>
                    {
                        this._stableSelection = this.listBox_audios.SelectedItems
                            .Cast<AudioObj>()
                            .ToList();
                    }));
                }
        ...
        private void listBox_audios_KeyDown(object? sender, KeyEventArgs e)
        {
            // Handle arrow key navigation for track selection (single selection only)
            // Important: Hide waveform preview first to ensure list box retains keyboard focus
            if (e.KeyCode == Keys.Up || e.KeyCode == Keys.Down)
            {
                // Hide any showing waveform preview to return focus to list box
                this.waveformPreviewTimer.Stop();
                this.waveformPreviewIndex = -1;
                this.HideWaveformPreview();

                int currentIndex = this.listBox_audios.SelectedIndex;
                ...
                {
                    newIndex = Math.Min(this.listBox_audios.Items.Count - 1, currentIndex + 1);
                }

                if (newIndex != currentIndex)
                {
                    // Ensure clean single selection - clear any existing selection first
                    this.listBox_audios.ClearSelected();
                    this.listBox_audios.SelectedIndex = newIndex;
                    this.listBox_audios.SetSelected(newIndex, true);
                }

                // If auto-play is checked, play the newly selected track
                if (this.checkBox_autoPlay.Checked && newIndex >= 0 && newIndex < this.listBox_audios.Items.Count)
                {
                    AudioObj audio = (AudioObj)this.listBox_audios.Items[newIndex];
                    _ = this.TriggerAutoPlayAsync(audio);
                }

                if (this.ShowPreview && newIndex >= 0 && newIndex < this.listBox_audios.Items.Count)
                {
                    this.waveformPreviewIndex = newIndex;
                    this.ShowWaveformPreviewForIndex(newIndex);
        ...
        private async Task TriggerAutoPlayAsync(AudioObj audio, bool toggleSameTrack = false)
        {
            ...
            try
            {
                ...
                if (toggleSameTrack)
                {
                    ...
                    {
                        sameTrack = ReferenceEquals(this.autoPlayCurrent, audio);
                    }
        ...
        private void ShowWaveformPreviewForIndex(int index)
        {
            ...
            if (this.listBox_audios.Items[index] is AudioObj audio && audio.WaveformPreview is Bitmap bmp)
            {
                if (this.waveformPreviewForm == null || this.waveformPreviewForm.IsDisposed)
                {
                    this.waveformPreviewForm = new WaveformPreview();
                ...
                Point screenPos = this.listBox_audios.PointToScreen(this.listBox_audios.GetItemRectangle(index).Location);
                screenPos.Offset(20, 10); // etwas rechts/unten von der ListBox
                this.waveformPreviewForm.ShowWaveform(bmp, screenPos);
            }
        }
