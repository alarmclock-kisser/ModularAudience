...
namespace ModularAudience.Audio.Processors_V1
{
	public static class LoopAtomizer
	{
		...
		{
			if (audioObj == null) throw new ArgumentNullException(nameof(audioObj));
			...
			if (interleaved.Length == 0) return Array.Empty<AudioObj>();
			...
			if (refined.Count == 0) refined.Add((0, mono.Length));
			...
			if (similarityHint.HasValue) clusterThreshold = Math.Clamp(similarityHint.Value, 0.6f, 0.98f);
			else clusterThreshold = DetermineAdaptiveClusterThreshold(fps, clusterThreshold);
		...
		private static AdaptiveParams DeriveAdaptiveParams(float[] mono, int sr, float? similarityHint, out bool jungleMode)
		{
			...
			double rms = 0; foreach (var v in mono) rms += v * v; rms = Math.Sqrt(rms / Math.Max(1, mono.Length));
			...
			try
			{
				...
				if (energy.Count > 8)
				{
					...
					{
						double corr = 0;
						for (int k = 0; k + lag < energy.Count; k++) corr += energy[k] * energy[k + lag];
			...
			if (rms < 1e-5) p.UseSpectralFlux = false;
			if (similarityHint.HasValue) p.ClusterSimilarityThreshold = Math.Clamp(similarityHint.Value, 0.6f, 0.98f);
			return p;
		}
		...
		private static List<int> DetectOnsets(float[] mono, int sr, AdaptiveParams p)
		{
			...
			int fftN = 1; while (fftN < win) fftN <<= 1;
			var buf = new Complex[fftN];
			double[]? prevMag = null;
			var w = new double[win];
			for (int i = 0; i < win; i++) w[i] = 0.5 * (1 - Math.Cos(2 * Math.PI * i / (win - 1)));
			...
			for (int f = 0; f < frames; f++)
			{
				...
				for (int i = win; i < fftN; i++) buf[i] = Complex.Zero;
				...
				if (p.UseSpectralFlux)
				{
					...
					for (int k = 0; k < mag.Length; k++) mag[k] = buf[k].Magnitude;
					if (prevMag != null)
					{
						double fl = 0;
						int mlen = Math.Min(prevMag.Length, mag.Length);
						for (int k = 0; k < mlen; k++) { double d = mag[k] - prevMag[k]; if (d > 0) fl += d; }
						flux[f] = fl;
					}
					prevMag = mag;
				}
			...
			if (p.UseSpectralFlux) NormalizeInPlace(flux);
			...
			{
				double s = smoothE[i];
				if (p.UseSpectralFlux) s = 0.65 * s + 0.35 * smoothF![i];
				score[i] = s;
			}
		...
		private static List<int> DensifyOnsetsForJungle(float[] mono, int sr, List<int> baseOnsets, AdaptiveParams ap)
		{
			...
			for (int f = 0; f < frames; f++)
			{
				int pos = f * hop;
				double s = 0;
				for (int i = 0; i < win; i++) s += Math.Abs((pos + i) < mono.Length ? mono[pos + i] * w[i] : 0.0);
				env[f] = s / win;
			}
		...
		private static List<(int start, int end)> BuildCandidateSegments(List<int> onsets, int totalSamples, int sr, AdaptiveParams p)
		{
			...
			if (onsets.Count == 0) return list;

			// median inter-onset
			var diffs = new List<int>();
			for (int i = 1; i < onsets.Count; i++) diffs.Add(onsets[i] - onsets[i - 1]);
		...
		{
			if (segments == null || segments.Count == 0) return new List<(int, int)>();
		...
		private static List<(int start, int end)> SplitAtValleys(float[] mono, int s, int e, int sr, int minSegmentSamples)
		{
			...
			if (e - last >= minSegmentSamples) outList.Add((last, e));
			else
			{
				// append to last
				if (outList.Count > 0)
		...
		{
			if (channels <= 1) return interleaved ?? Array.Empty<float>();
		...
		private static void NormalizeInPlace(double[] arr)
		{
			...
			double span = mx - mn;
			if (span <= 1e-9) return;
			for (int i = 0; i < arr.Length; i++) arr[i] = (arr[i] - mn) / span;
		}
		...
		private static List<int> PickPeaksWithThreshold(double[] arr, int neighbor, int minSpacingFrames, double threshold)
		{
			...
			foreach (var p in candidates)
			{
				...
				{
					// replace if stronger
					if (filtered.Count > 0 && arr[p] > arr[filtered.Last()]) filtered[filtered.Count - 1] = p;
				}
		...
		private static List<int> FallbackPartition(int totalSamples, int sr)
		{
			...
			for (int i = 0; i < parts; i++) res.Add(Math.Clamp((int) Math.Round(i * (totalSamples / (double) parts)), 0, totalSamples - 1));
			return res;
		}
		...
		private static int FindNearestZeroCrossingIndexSafe(int pos, int offset, int totalSamples)
		{
			...
			if (target >= totalSamples) target = totalSamples - 1;
		...
		{
			if (segs == null || segs.Count == 0) return new List<(int, int)>();
		...
		private static AudioObj IsolateAndFinalize(AudioObj sample, int sampleRate)
		{
			...
			if (data.Length == 0) return sample.Clone();
			...
			float peak = 0f; for (int i = 0; i < outData.Length; i++) peak = MathF.Max(peak, MathF.Abs(outData[i]));
			const float targetPeak = 0.95f;
			if (peak > 1e-8f && peak > targetPeak)
			{
				float scale = targetPeak / peak;
				for (int i = 0; i < outData.Length; i++) outData[i] *= scale;
			}
		...
		private static double[] ComputeSpectralFingerprint(float[] interleaved, int channels, int sampleRate, int bins = 128)
		{
			var mono = ToMono(interleaved, channels);
			int N = 1;
			while (N < mono.Length && N < 8192) N <<= 1;
			if (N < 256) N = 256;
			var window = new double[N];
			for (int i = 0; i < N; i++) window[i] = 0.5 * (1 - Math.Cos(2 * Math.PI * i / (N - 1)));
			var buf = new Complex[N];
			int offset = Math.Max(0, mono.Length / 2 - N / 2);
			for (int i = 0; i < N; i++) buf[i] = new Complex((i + offset) < mono.Length ? mono[i + offset] * window[i] : 0.0, 0.0);
			Fourier.Forward(buf, FourierOptions.Matlab);
			int half = N / 2;
			var mags = new double[half];
			double max = 1e-12;
			for (int k = 0; k < half; k++) { mags[k] = buf[k].Magnitude; if (mags[k] > max) max = mags[k]; }
			...
			for (int b = 0; b < bins; b++)
			{
				...
				double sum = 0; for (int k = a; k < bb; k++) sum += mags[k];
		...
		{
			if (interleaved == null || interleaved.Length == 0) return 0.0;
			var mono = ToMono(interleaved, channels);
			double s = 0; foreach (var v in mono) s += v * v;
		...
		private static void ApplyLinearFade(float[] interleaved, int channels, int fadeSamples)
		{
			...
			for (int f = 0; f < fadeSamples; f++)
			{
				...
				for (int c = 0; c < channels; c++) interleaved[f * channels + c] *= g;
			}
			for (int f = 0; f < fadeSamples; f++)
			{
				int idx = frames - fadeSamples + f;
				float g = 1.0f - f / (float) fadeSamples;
				for (int c = 0; c < channels; c++) interleaved[idx * channels + c] *= g;
			}
		}
		...
		{
			int len = e - s;
			if (len < 32) return 0.0;
			int N = 1;
			while (N < len) N <<= 1;
			if (N > 8192) N = 8192;
			var buf = new Complex[N];
			for (int i = 0; i < N; i++) buf[i] = new Complex((s + i) < mono.Length ? mono[s + i] : 0.0, 0.0);
			Fourier.Forward(buf, FourierOptions.Matlab);
			double num = 0, den = 0;
			int half = N / 2;
